...
public class SimpleTextStatisticsAnalyzer implements TextStatisticsAnalyzer {
    ...
    public List<String> sortWordsByLength(String text, Direction direction) {
        List<String> word = new ArrayList<>(getWords(text));
        if (direction.equals(Direction.ASC)) {
            Collections.sort(word, Comparator.comparing(String::length));
        } else {
            Collections.sort(word, Comparator.comparing(String::length).reversed());
        }
        return word;
    }
}
